...
- **Error reporting**: If a date cannot be parsed, the user is informed
  of the provided date and associated row ID, allowing for easier
...
to clean multiple date columns simultaneously:
...
[`fix_date_df()`](https://docs.ropensci.org/datefixR/reference/fix_date_df.md)
now supports parallelism over columns via the `cores` argument (or via
the `'Ncpus'` global option). As such, speed is now very unlikely to be
an issue when using `datefixR` on large datasets.
...
1.  When a date fails to parse in
    [lubridate](https://lubridate.tidyverse.org) then the user is simply
...
2.  When imputing a missing day or month, there is no user-control over
    ...
    However, January will always be imputed in
    [lubridate](https://lubridate.tidyverse.org). In `datefixR`, this
    behavior can be controlled by the `month.impute` argument.
...
However, [lubridate](https://lubridate.tidyverse.org) of course excels
in general date manipulation and is an excellent tool to use alongside
`datefixR`.
...
also attempts to solve the problem of handling arbitrary dates and
parses dates into the `POSIXct` type. Unfortunately, `parse_date()`
cannot handle years before 1970 – instead imputing the year as the
current year without any warnings being raised.
...
allow the user to specify otherwise. However,
[parsedate](https://github.com/gaborcsardi/parsedate) has excellent
support for handling dates in ISO 8601 formats.

### `stringi`, `readr`, and `clock`

These packages all use [ICU
library](https://unicode-org.github.io/icu/userguide/format_parse/datetime/)
when parsing dates (via
[`stringi::stri_datetime_parse()`](https://rdrr.io/pkg/stringi/man/stri_datetime_format.html),
[`readr::parse_date()`](https://readr.tidyverse.org/reference/parse_datetime.html),
or `clock::date_parse()`) and therefore all behave very similarly.
Notably, all of these functions require the date format to be specified
including specifying a priori if a date is missing. Ultimately, this
makes these packages unsuitable when numerous dates in different formats
must be parsed.
